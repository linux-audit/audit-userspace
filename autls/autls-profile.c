...
/*
 * Default TLS 1.3 ciphersuite string for compatible and PQC profiles.
...
/* Compatible key exchange groups -- includes PQC hybrid with fallback */
#define AUTLS_COMPATIBLE_GROUPS	"X25519MLKEM768:X25519"
 ...
 * @profile: one of AUTLS_PROFILE_COMPATIBLE, AUTLS_PROFILE_SYSTEM,
 *           AUTLS_PROFILE_PQC
 *
 * COMPATIBLE and PQC return the default TLS 1.3 ciphersuites.
 * SYSTEM returns NULL (defer to system crypto policy).
 */
const char *autls_profile_ciphers(int profile)
{
	switch (profile) {
	case AUTLS_PROFILE_COMPATIBLE:
	case AUTLS_PROFILE_PQC:
		return AUTLS_DEFAULT_CIPHERS;
	case AUTLS_PROFILE_SYSTEM:
		return NULL;
	default:
		return AUTLS_DEFAULT_CIPHERS;
	}
 ...
 *           AUTLS_PROFILE_PQC
 *
 * COMPATIBLE returns hybrid-plus-classical groups with fallback.
